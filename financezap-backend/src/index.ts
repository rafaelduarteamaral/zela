...
// Número do WhatsApp de destino (formato: whatsapp:+5511999999999)
const YOUR_WHATSAPP_NUMBER = 'whatsapp:+5511999999999'; // Exemplo genérico
...
async function ativarAssinaturaUsuario(telefone: string, planoId: string) {
  ...
  if (resultado.count === 0) {
    throw new Error('Usuário não encontrado para ativar assinatura');
  }
...
app.post('/api/auth/login', async (req, res) => {
  ...
  try {
    ...
    const formatos = [
      ...
      `+${telefoneLimpo}`,                      // +11999999999 (se já tiver 55)
...
app.post('/api/payments/abacatepay/checkout', autenticarMiddleware, async (req: any, res) => {
  try {
    if (!process.env.ABACATEPAY_API_KEY) {
      return res.status(500).json({ success: false, error: 'ABACATEPAY_API_KEY não configurada' });
    }
    ...
    console.error('Erro ao criar cobrança Abacate Pay:', error);
    return res.status(500).json({ success: false, error: error.message || 'Erro ao criar cobrança' });
  }
});
  ...
  try {
    ...
    if (cobranca.metadata?.telefone && cobranca.metadata?.planoId && cobranca.status === 'paid') {
      try {
        ...
        console.warn('Cobrança paga, mas não foi possível ativar assinatura automaticamente:', err);
      }
    }
    ...
    console.error('Erro ao consultar cobrança Abacate Pay:', error);
    return res.status(500).json({ success: false, error: error.message || 'Erro ao consultar cobrança' });
  }
});
...
app.post('/webhooks/abacatepay', async (req: any, res) => {
  try {
    ...
    if (!secretOk) {
      return res.status(401).json({ success: false, error: 'Webhook secret inválido' });
    }
